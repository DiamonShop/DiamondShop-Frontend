...
const Don_Hang = () => {
    ...
    const columns = [
        ...
        { title: 'Ngày đặt đơn', dataIndex: 'orderDate', key: 'orderDate' },
        { title: 'Tổng cộng', dataIndex: 'totalPrice', key: 'totalPrice' },
        { title: 'Trạng thái', dataIndex: 'status', key: 'status', className: getStatusClass },
        {
            title: 'Hành động',
            key: 'action',
            render: (text, record) => (
                <span>
                    <button type="button" className='admin-page-view-button' onClick={() => handleViewDetails(record)}>Xem chi tiết</button>
                    {record.status !== 'Completed' && record.status !== 'Cancel' && (
                        <button type="button" className='admin-page-delete-button' onClick={() => handleCancelClick(record)}>Hủy đơn hàng</button>
                    )}
                </span>
            ),
        },
    ...
    const fetchUserData = async () => {
        ...
        try {
            ...
            // Get all Orders
            const ordersResponse = await axios.get('https://localhost:7101/api/orders/GetAllOrders', {
                headers: {
                    ...headers,
                    'Content-Type': 'application/json'
                }
                ...
                <div className="content">
                    ...
                    {selectedOrder && (
                        ...
                        >
                            ...
                            <p><strong>Họ và tên:</strong> {selectedOrder.userName}</p>
                            <p><strong>Tổng cộng:</strong> {selectedOrder.totalPrice}</p>
                            <p><strong>Trạng thái:</strong> {selectedOrder.status}</p>
                            <p><strong>Ngày đặt đơn:</strong> {selectedOrder.orderDate}</p>
                            <p><strong>Ghi chú:</strong> {selectedOrder.orderNote}</p>
                    ...
                    {showCancelPopup && (
                        <Modal
                            ...
                            footer={[
                                <Button key="back" onClick={() => setShowCancelPopup(false)}>Hủy bỏ</Button>,
                                <Button key="submit" type="primary" onClick={handleCancelOrder}>Xác nhận</Button>,
                            ]}
                        >
